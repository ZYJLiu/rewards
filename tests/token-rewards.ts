...
import { findMetadataPda, mintToBuilder } from "@metaplex-foundation/js"
...
describe("token-rewards", () => {
  ...
  before(async () => {
    ...
    ;[rewardMintPDA] = await PublicKey.findProgramAddress(
      [Buffer.from("REWARD"), merchantPDA.toBuffer()],
      program.programId
    )

    // await connection.requestAirdrop(userWallet.publicKey, 2)
